...
import { Home, Users, Star, BookOpen, Zap, Calendar, LifeBuoy, Edit3, MessageSquare, ThumbsUp } from "lucide-react"
import Link from "next/link"
import { ACTBotV3 } from "@/components/act-bot-v3" // For embedded bot in Support tab
import { submitContactForm, submitTweakRequest, schedulePaidConsult } from "@/app/actions" // Mock server actions
...
const mockUser = {
  ...
  purchases: [
    { id: "purchase1", name: "Growth Bundle", date: "2025-05-15", tweakUsed: false, daysRemainingForTweak: 15 },
    { id: "purchase2", name: "Playbook", date: "2025-04-10", tweakUsed: true, daysRemainingForTweak: 0 },
  ],
  hoursSaved: 12,
...
const reviewPlatforms = ["LinkedIn", "Google Reviews", "Facebook", "Instagram", "Yelp", "Nextdoor"]
...
export function MembersArea({ startBotAt }: MembersAreaProps) {
  ...
  // Tweak Page State
  const [selectedPurchaseForTweak, setSelectedPurchaseForTweak] = useState<(typeof userData.purchases)[0] | null>(null)
  const [tweakChallenge, setTweakChallenge] = useState(userData.q5_challenge || "")
  ...
  const totalReviewCredits = userData.reviews.filter((r) => r.submitted).reduce((sum, r) => sum + (r.credit || 0), 0)
  ...
  const handleTweakSubmit = async (e: React.FormEvent) => {
    ...
    if (tweakAdditionalNotes.length < 200) {
      setTweakFormError("Please provide more detail for additional notes (min 200 characters).")
      return
    }
    setTweakFormError("")
    setTweakSuccessMessage("")
    ...
    try {
      ...
      if (result.success) {
        setTweakSuccessMessage(
          result.message || "Tweak submitted successfully! Your updated deliverable will be emailed.",
        )
        // Update userData to reflect tweak used for this purchase
        setUserData((prev) => ({
          ...prev,
          purchases: prev.purchases.map((p) => (p.id === selectedPurchaseForTweak.id ? { ...p, tweakUsed: true } : p)),
        }))
  ...
  async function handleSupportFormSubmit(formData: FormData) {
    ...
    try {
      await submitContactForm({ name, email, message, pageSource: "Members Area Support" })
      ...
      <header className="bg-white border-b p-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link href="/" className="flex items-center">
            ...
            <span className="ml-2 text-gray-800 text-lg">Kefford Consulting</span>
          </Link>
          <div className="flex items-center space-x-4">
            <span className="text-sm text-gray-600 hidden md:inline">Welcome, {userData.name}</span>
            <Button variant="outline" size="sm" onClick={() => alert("Logout (mocked)")}>
              Logout
            </Button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl h-[80vh] rounded-lg shadow-2xl overflow-hidden flex flex-col">
            <div className="p-3 border-b flex justify-between items-center bg-red-50">
              <h3 className="font-semibold text-red-700">Continue Your Growth Journey</h3>
              <Button
                variant="ghost"
                size="sm"
        ...
        <Tabs defaultValue="dashboard" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2 bg-gray-200 p-1 rounded-lg">
            {[
              ...
              >
                <tab.icon size={18} className="mb-1 md:mb-0 md:mr-2" /> {tab.label}
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          <TabsContent value="dashboard">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl text-red-700">Dashboard</CardTitle>
                <CardDescription>Your Kefford Consulting impact and activity.</CardDescription>
              </CardHeader>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            You were in the middle of our consultation. Would you like to continue?
                          </p>
                        </div>
                        <Button
                    ...
                    <CardContent className="space-y-3">
                      <div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-bold text-blue-600">{userData.hoursSaved} Hrs</span>
                        </div>
                        <Progress
                      ...
                      <div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-bold text-green-600">${userData.creditsEarned}</span>
                        </div>
                        <Progress
                          value={(userData.creditsEarned / MAX_REVIEW_CREDITS) * 100}
                          className="h-2 mt-1"
                          indicatorClassName="bg-green-600"
                        />
                        <p className="text-xs text-gray-500 mt-0.5">Next Reward: $500 Cashback at 5 Referrals</p>
                      </div>
                    </CardContent>
                  </Card>
                  <Card>
                    ...
                    <CardContent>
                      <p className="text-sm text-gray-600">
                        Based on your {userData.q5_challenge} needs, try this Quick Win:
                      </p>
                      <Button
                        variant="link"
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <ul className="list-disc list-inside text-sm space-y-1 text-gray-700">
                        ...
                        <li className="text-red-500">Next: Schedule Consult (if applicable)</li>
                      </ul>
                    </CardContent>
                  </Card>
                </div>
                <div>
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    ...
                    {userData.purchases
                      ...
                      .map((p) => (
                        <li key={`${p.id}-tweak`}>
                          Tweak Completed for {p.name} on {new Date().toLocaleDateString()} (mock date)
                        </li>
                      ))}
          ...
          <TabsContent value="referral">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl text-red-700">Referral Program</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <p className="text-gray-700">
                  Invite friends to Kefford Consulting and earn cashback! Get $100 for each of the first 5 successful
                  referrals (up to $500).
                </p>
                <div className="p-4 bg-red-50 rounded-md text-center">
                  <p className="text-lg font-semibold">
                    Your Referral Code: <span className="text-red-600 tracking-wider">{userData.referralCode}</span>
                  </p>
                </div>
                <div className="flex gap-4">
                  <Button
                    onClick={() => alert(`Share code ${userData.referralCode} on Facebook (mocked)`)}
                    className="bg-blue-600 hover:bg-blue-700 text-white"
                  >
                    Share on Facebook
                  </Button>
                  <Button
                    onClick={() => alert(`Share code ${userData.referralCode} on Instagram (mocked)`)}
                ...
                <div>
                  <h4 className="text-lg font-semibold mb-2">Referral Status: {userData.referrals} / 5</h4>
                  <Progress value={(userData.referrals / 5) * 100} className="h-3" indicatorClassName="bg-red-500" />
                </div>
                <div>
                  <h4 className="text-lg font-semibold mb-2">Top Referrers This Month (Anonymized)</h4>
          ...
          <TabsContent value="reviews">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl text-red-700">Review Progress & Rewards</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <p className="text-gray-700">
                  Share your experience with Kefford Consulting on these platforms and earn $25 credit for each review
                  (up to $150 total). Credits can be used for future services or consults.
                </p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {reviewPlatforms.map((platform) => {
                    const review = userData.reviews.find((r) => r.platform === platform)
                    return (
                      <Card
                        key={platform}
                        className={cn(
                          "p-4 text-center",
                          review?.submitted ? "bg-green-50 border-green-500" : "bg-gray-50",
                        )}
                      >
                        <h5 className="font-semibold mb-2">{platform}</h5>
                        {review?.submitted ? (
                          <p className="text-sm text-green-700">
                            <ThumbsUp size={14} className="inline mr-1" /> Submitted! (+${review.credit} Credit)
                          </p>
                        ) : (
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => alert(`Submit review for ${platform} (mocked). Ensure FTC disclosure.`)}
                          >
                            Submit Review
                          </Button>
                        )}
                ...
                <Button
                  ...
                  onClick={() => alert(`Redeem ${totalReviewCredits} credits (mocked).`)}
                >
                  Redeem Credits
                </Button>
              </CardContent>
          ...
          <TabsContent value="content">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl text-red-700">Exclusive Content</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="text-center py-10 bg-gray-100 rounded-lg">
                  ...
                  <p className="text-gray-600">In-depth articles, videos, and resources tailored for your growth.</p>
                </div>
                <form
                  action={async (formData) => {
                    ...
                    if (topic) {
                      ...
                      ;(document.getElementById("topic_request_form") as HTMLFormElement)?.reset()
                    } else {
                ...
                >
                  <h4 className="font-semibold mb-2">What topic would you like to see?</h4>
                  <Textarea
                    name="topic_request"
          ...
          <TabsContent value="wins">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl text-red-700">Quick Wins</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="text-center py-10 bg-gray-100 rounded-lg">
                  ...
                  <p className="text-gray-600">
                    Actionable quick wins to boost your business immediately. E.g., "5 Ways to Boost Sales," "Daily
          ...
          <TabsContent value="services">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl text-red-700">Kefford Consulting Services</CardTitle>
              </CardHeader>
              <CardContent className="space-y-8">
                {/* Calendar Booking */}
                <div>
                  <h3 className="text-xl font-semibold mb-3">Schedule a Consult Call</h3>
                  <p className="text-sm text-gray-600 mb-3">Book a 30-minute paid consult call with Edwin for $300.</p>
                  <Button onClick={handleSchedulePaidConsult} className="bg-red-600 hover:bg-red-700 text-white">
                    <Calendar size={16} className="mr-2" />
                    Book Paid Consult
                  </Button>
                  <p className="text-xs text-gray-500 mt-1">
                    Google Calendar integration (mocked). You'll be redirected to Stripe for payment before
                    confirmation.
                  </p>
                </div>
                <hr />
                {/* Tweak Page Section */}
                <div>
                  <h3 className="text-xl font-semibold mb-1">Manage Your Plan Tweaks</h3>
                  <p className="text-sm text-gray-600 mb-4">
                    Submit your one-time tweak request for eligible purchases within 30 days.
                  </p>
                    ...
                    <Card key={purchase.id} className="mb-4 p-4 bg-gray-50">
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">Purchased on: {purchase.date}</p>
                          {purchase.tweakUsed ? (
                            <p className="text-xs text-red-500 font-medium">Tweak already used for this purchase.</p>
                          ) : purchase.daysRemainingForTweak > 0 ? (
                            <p className="text-xs text-green-600">
                              Days remaining for tweak: {purchase.daysRemainingForTweak}
                            </p>
                          ) : (
                            <p className="text-xs text-gray-500">Tweak window expired.</p>
                          )}
                        </div>
                        <Button
                          size="sm"
                          onClick={() => handleStartTweak(purchase)}
                          disabled={purchase.tweakUsed || purchase.daysRemainingForTweak <= 0}
                  ...
                  {selectedPurchaseForTweak && (
                    <Card className="mt-6 p-6 border-red-500 border-2">
                      ...
                      <form onSubmit={handleTweakSubmit} className="space-y-4">
                        ...
                        <div>
                          <label className="text-sm font-medium">Update business size (if changed):</label>
                          <Input
                            value={tweakBusinessSize}
                            onChange={(e) => setTweakBusinessSize(e.target.value)}
                        ...
                        <div>
                          <label className="text-sm font-medium">
                            Update your dream outcome, success metric, and settle point:
                          </label>
                          <Textarea
                            value={tweakDreamOutcome}
                            onChange={(e) => setTweakDreamOutcome(e.target.value)}
                        ...
                        <div>
                          <label className="text-sm font-medium">
                            Any additional changes or notes for your tweak? (min 200 chars)
                          </label>
                          <Textarea
                            value={tweakAdditionalNotes}
                            onChange={(e) => setTweakAdditionalNotes(e.target.value)}
                            rows={5}
                            placeholder="E.g., I'd like to focus more on lead generation strategies..."
                            className="mt-1"
                          />
                          {tweakFormError && <p className="text-xs text-red-500 mt-1">{tweakFormError}</p>}
                        </div>
                        <Button type="submit" className="bg-green-600 hover:bg-green-700 text-white">
                          Confirm Tweak & Redeliver
                        </Button>
                        {tweakSuccessMessage && <p className="text-sm text-green-600 mt-2">{tweakSuccessMessage}</p>}
                      </form>
                      <div className="mt-6 border-t pt-4">
                        <p className="text-sm text-gray-700 mb-2">Want expert guidance on this tweak?</p>
                        <Button onClick={handleSchedulePaidConsult} className="bg-red-600 hover:bg-red-700 text-white">
                ...
                {/* Plans PDF Access */}
                <div>
                  <h3 className="text-xl font-semibold mb-3">Your Purchased Plans</h3>
                  {userData.purchases.length > 0 ? (
                    userData.purchases.map((p) => (
                      <div key={p.id} className="flex justify-between items-center p-3 bg-gray-100 rounded-md mb-2">
                        <span className="text-sm font-medium">{p.name}</span>
                        <Button variant="outline" size="sm" onClick={() => alert(`Download ${p.name}.pdf (mocked)`)}>
                          Download PDF
                        </Button>
                      </div>
                    ))
          ...
          <TabsContent value="support">
            <Card>
              ...
              <CardContent className="space-y-6">
                <p className="text-gray-700">
                  Need help? Fill out the form below or chat with our ACT Bot for quick assistance.
                </p>
                <form action={handleSupportFormSubmit} className="space-y-4 p-4 border rounded-md bg-gray-50">
                  <div>
                    <label htmlFor="support_message" className="block text-sm font-medium text-gray-700">
                      Your Message
                    </label>
                    <Textarea
                ...
                <div className="text-center mt-6">
                  ...
                  >
                    <MessageSquare size={16} className="mr-2" /> Chat with ACT Bot Now
                  </Button>
                </div>
                {showSupportBot && (
                  <div
                  ...
                  >
                    ...
                    >
                      <div className="p-2 border-b flex justify-end">
                        {" "}
                        <Button variant="ghost" size="sm" onClick={() => setShowSupportBot(false)}>
                          Close
                        </Button>
                      </div>
